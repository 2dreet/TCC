...
import dialog.DialogJogadorPartida;
...
public class TelaLogin extends JFrame {
		...
		logar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(login.getText() == null || login.getText().isEmpty()){
					...
					senha.requestFocus();
				}else if (UsuarioDao.logar(login.getText(), senha.getText(), 2)) {
					try {
				...
				} else if (UsuarioDao.logar(login.getText(), senha.getText(), 3)) {
					try {
						senhaOld = senha.getText();
						loginOld = login.getText();
						logado();
	...
	public void logado() throws Throwable {
		...
		if (senhaOld.equals("123")) {
			...
			if (UsuarioDao.usuarioLogin(loginOld, senhaOld, 3) != null) {
				u = UsuarioDao.usuarioLogin(loginOld, senhaOld, 3);
				rs = new RedefinirSenha(senhaOld, UsuarioDao.usuarioLogin(
							loginOld, senhaOld, 3), this);
			} else {
				rs = new RedefinirSenha(senhaOld, UsuarioDao.usuarioLogin(
							loginOld, senhaOld, 2), this);
				u = UsuarioDao.usuarioLogin(loginOld, senhaOld, 2);
			}
			rs.setVisible(true);
			alterou = rs.alterada;
			if (rs.alterada) {
		...
		if(u != null){
		dispose();
		Login.usuario = FuncionarioDao.getFuncionarioUsurio(u.getCodigoUsuario());
		Home home = new Home();
