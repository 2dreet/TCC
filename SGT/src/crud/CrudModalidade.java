...
import dialog.DialogCrudModalidade;
...
public class CrudModalidade extends JPanel{
	...
	public CrudModalidade(MenuConfiguracoes menuPai){
		...
		lblHeader = new JLabel("Modalidade");
		...
		btAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				DialogCrudModalidade c = new DialogCrudModalidade();
				c.crudModalidade(null, ParametroCrud.getModoCrudNovo(),meio);
				if(c.getConfirmado()){
					Menssage.setMenssage("Nova Modalidade", "Modalidade cadastrada com Sucesso!", ParametroCrud.getModoCrudNovo(), meio);
					localizar();
				}
			}
		});
		...
		btAlt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(selecionar()){
					DialogCrudModalidade c = new DialogCrudModalidade();
					c.crudModalidade(modalidadeSelecionada, ParametroCrud.getModoCrudAlterar(),meio);
					if(c.getConfirmado()){
						Menssage.setMenssage("Altera Modalidade", "Modalidade alterada com Sucesso!", ParametroCrud.getModoCrudAlterar(), meio);
						localizar();
					}
				}
			}
		...
		btDel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(selecionar()){
					DialogCrudModalidade c = new DialogCrudModalidade();
					c.crudModalidade(modalidadeSelecionada, ParametroCrud.getModoCrudDeletar(),meio);
					if(c.getConfirmado()){
						Menssage.setMenssage("Deletar Modalidade", "Modalidade deletada com Sucesso!", ParametroCrud.getModoCrudDeletar(), meio);
						localizar();
					}
				}
			}
	...
	public boolean selecionar() {
		if (tabela.getRowCount() > 0) {
			if (tabela.getSelectedRow() > -1) {
				modalidadeSelecionada = ModalidadeDao
						.getModalidade(Integer.parseInt(String.valueOf(tabela
								.getValueAt(tabela.getSelectedRow(), 0))));
				return true;
			} else {
				Menssage.setMenssage("Modalidade não Selecionada",
						"Deve selecionar uma Modalidade!",
	...
	public void localizar() {
		listaModalidade = ModalidadeDao.getListaPesquisa(metodoBusca.getSelectedItem()
